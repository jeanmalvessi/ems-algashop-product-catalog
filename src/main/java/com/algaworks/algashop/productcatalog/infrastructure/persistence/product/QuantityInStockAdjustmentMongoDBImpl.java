...
import com.algaworks.algashop.productcatalog.domain.model.product.QuantityInStockAdjustment;
...
public class QuantityInStockAdjustmentMongoDBImpl implements QuantityInStockAdjustment {
    ...
    public Result decrease(UUID productId, int quantity) {
        Query query = Query.query(Criteria.where("id").is(productId).and("quantityInStock").gte(quantity));
